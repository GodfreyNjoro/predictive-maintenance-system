...
import { DataSourceDrawer } from "../../../data-sources/data-source-drawer";
import { IngestionRunsPanel } from "../../../data-sources/ingestion-runs-panel";
import type { DataSourceRow } from "../../../data-sources/page";

export function DataSourcesTab(props: {
  applicationId: string;
  initialAction: "create" | null;
}) {
  const { applicationId, initialAction } = props;
  ...
  async function fetchAll() {
    setLoading(true);
    try {
      const r = await fetch(`/api/data-sources?applicationId=${applicationId}`, {
        cache: "no-store",
      });
  ...
  // Auto-open drawer when redirected from wizard with ?action=create
  useEffect(() => {
    if (initialAction === "create" && !autoOpenedRef && !loading) {
      setEditing(null);
      setDrawerOpen(true);
      setAutoOpenedRef(true);
    }
  }, [initialAction, autoOpenedRef, loading]);
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-slate-500">Database connections that feed predictive signals for this application.</p>
        </div>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-slate-600 mt-1">
                    Removes the connection, its watermarks, and ingestion runs. Already-ingested ParsedLog rows are preserved.
                  </p>
                </div>
              </div>
...
function EmptyState({ onAdd }: { onAdd: () => void }) {
  return (
    <div className="rounded-xl border-2 border-dashed border-slate-200 bg-white p-12 text-center">
      ...
      <h3 className="font-semibold text-slate-900 text-lg">No data sources yet</h3>
      <p className="text-sm text-slate-600 mt-1 max-w-md mx-auto">
        Connect a database (SQL Server, Postgres, MySQL, Oracle) to start streaming telemetry into the predictive pipeline.
      </p>
      <button
        onClick={onAdd}
  ...
  return (
    ...
    >
      ...
      <div className="p-5 space-y-3 text-sm">
        ...
        <Row
          label="Feeds enabled"
          value={row.enabledFeeds.length === 0 ? "jobHistory (default)" : row.enabledFeeds.join(", ")}
          icon={Activity}
        />
      ...
      <div className="px-5 py-3 bg-slate-50 border-t border-slate-100 flex items-center justify-between rounded-b-xl">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-medium bg-white border border-slate-200 text-slate-700 hover:bg-slate-100 disabled:opacity-50"
          >
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-medium bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50"
          >
...
function authLabel(a: string): string {
  switch (a) {
    ...
    case "WINDOWS_AUTH":
      return "Windows Authentication (NTLM)";
    case "IAM":
      return "IAM";
    case "TOKEN":
      return "Token";
